...
def get_model(sensor : Dict, N : int, input_features: int = 1,
    ...
    # LSTM 
    x = layers.LSTM(round(N), input_shape = (N, input_features), return_sequences = True, dropout=0.05)(input)
    x = layers.LSTM(round(N/2), input_shape = (N, input_features), return_sequences = True, dropout=0.05)(x)
    x = layers.LSTM(round(N/2), input_shape = (N, input_features), return_sequences = True,  dropout=0.05)(x)
    x = layers.LSTM(round(N/4), input_shape = (N, input_features), return_sequences = True,  dropout=0.05)(x)
    x = layers.LSTM(round(N/4), input_shape = (N, input_features), dropout=0.05)(x)
